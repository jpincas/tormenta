...
func Test_BasicQuery_DateRange(t *testing.T) {
	// Create a list of tts over a date range
	...
	//Quick check that all tts have saved correctly
	var results []TestType
	n, _, _ := db.Find(&results).Run()

	if len(results) != len(tts) || n != len(tts) {
		t.Errorf("Testing range query. Haven't even got to ranges yet. Just basic query expected %v - got %v/%v", len(tts), len(results), n)
		t.FailNow()
	}
	...
	testCases := []struct {
		...
		{"from right now - no tts expected, no 'to'", time.Now(), time.Time{}, 0, false, 0, 0},
		{"from beginning of time - all tts should be included, no 'to'", time.Time{}, time.Time{}, len(tts), false, 0, 0},
		{"from beginning of time - offset 1", time.Time{}, time.Time{}, len(tts) - 1, false, 0, 1},
		{"from beginning of time - offset 2", time.Time{}, time.Time{}, len(tts) - 2, false, 0, 2},
		...
		{"from beginning of time to now - expect all", time.Time{}, time.Now(), len(tts), true, 0, 0},
		{"from beginning of time to 2014 - expect 5", time.Time{}, time.Date(2014, time.January, 1, 1, 0, 0, 0, time.UTC), 5, true, 0, 0},
		{"from beginning of time to an hour ago - expect all but 1", time.Time{}, time.Now().Add(-1 * time.Hour), len(tts) - 1, true, 0, 0},
		...
		{"from beginning of time - limit more than there are", time.Time{}, time.Time{}, len(tts), false, 0, 0},
	}
	...
	for _, testCase := range testCases {
		...
		if rn != testCase.expected {
			t.Errorf("Testing REVERSE %s (number tts retrieved). Expected %v - got %v", testCase.testName, testCase.expected, rn)
		}
