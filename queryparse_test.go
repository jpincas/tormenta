...
func Test_BuildQuery(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"match - correct",
			"where=index:indexstring,match:1",
			db.Find(&results).Match("indexstring", 1),
			true,
			false,
		},
		{
			"match - incorrect",
			"where=index:indexstring,match:2",
			db.Find(&results).Match("indexstring", 1),
			false,
			false,
		},
		...
		// Range
		{
			...
			"where=index:indexstring,start:1",
			db.Find(&results).Range("indexstring", 1, nil),
			true,
			false,
		},
		{
			"range - end only - correct",
			"where=index:indexstring,end:100",
			db.Find(&results).Range("indexstring", nil, 100),
			true,
			false,
		},
		{
			"range - start and end - correct",
			"where=index:indexstring,start:1,end:100",
		...
		},
		{
			...
			"where=index:indexstring,start:1,end:invalidword",
			db.Find(&results),
			true,
			true,
		},
		{
			...
			"where=index:indexstring,start:1,end:100,match:1",
			db.Find(&results),
			true,
			true,
		},
		...
		},
		{
			...
			"limit=1&offset=1&reverse=true&where=index:indexstring,match:1",
			db.Find(&results).Limit(1).Offset(1).Reverse().Match("indexstring", 1),
			true,
			false,
		},
		{
			"limit, offset, reverse, index range",
			"limit=1&offset=1&reverse=true&where=index:indexstring,start:1,end:10",
		...
		},
		{
			...
			"limit=1&offset=1&reverse=true&where=index:indexstring,start:1,end:10&where=index:indexstring,startswith:test",
			db.Find(&results).Limit(1).Offset(1).Reverse().Range("indexstring", 1, 10).StartsWith("indexstring", "test"),
			true,
			false,
		},
		{
			"limit, offset, reverse, index range, startswith, or",
			"or=true&limit=1&offset=1&reverse=true&where=index:indexstring,start:1,end:10&where=index:indexstring,startswith:test",
			db.Find(&results).Limit(1).Offset(1).Reverse().Range("indexstring", 1, 10).Or().StartsWith("indexstring", "test"),
			true,
			false,
		},
	}
