...
func Test_BuildQuery(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			"limit=1&offset=1&reverse=true&where=index:IntField,start:1,end:10&where=index:IntField,startswith:test",
			db.Find(&results).Limit(1).Offset(1).Reverse().Range("IntField", 1, 10).StartsWith("IntField", "test"),
			true,
			false,
		},
		{
			"limit, offset, reverse, index range, startswith, or",
			"or=true&limit=1&offset=1&reverse=true&where=index:IntField,start:1,end:10&where=index:IntField,startswith:test",
			db.Find(&results).Limit(1).Offset(1).Reverse().Range("IntField", 1, 10).Or().StartsWith("IntField", "test"),
			true,
			false,
		},
	}
